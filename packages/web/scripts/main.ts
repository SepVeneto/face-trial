...
class Glass extends RigidBody {
  ...
  constructor(width: number, cx: number, cy: number, rotate: number) {
    super()
    this.width = width
    this.centerX = cx;
    this.centerY = cy;
...
window.onload = () => {
  ...
  canvas.addEventListener('mousedown', (evt: MouseEvent) => {
    ...
    canvas.addEventListener('mouseup', () => {
      (canvas as HTMLCanvasElement).removeEventListener('mousemove', handleMove)
    })
  })
...
export function renderer(face: any) {
  glass.update(face)
  leftLeg.update(face)
  scene.run()
}

export async function init(img?: string, face?: any) {
  let faceData = face
  if (img) {
...
async function handleSelect(evt: MouseEvent) {
